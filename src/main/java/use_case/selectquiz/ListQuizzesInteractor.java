    ...
    public void execute(ListQuizzesInputData inputData) {
        try {
            ...
            for (QuizItemDto item : all) {
                if (category != null && !"Any".equalsIgnoreCase(category)) {String c = item.getCategory();
                    if (c == null || !category.equalsIgnoreCase(c)) {continue;
                    }
                }

                if (difficulty != null && !"Any".equalsIgnoreCase(difficulty)) {String d = item.getDifficulty();
                    if (d == null || !difficulty.equalsIgnoreCase(d)) {continue;
                    }
                }
                if (type != null && !"Any".equalsIgnoreCase(type)) {String t = item.getType();
                    if (t == null || !type.equalsIgnoreCase(t)) {continue;
                    }
                }
                filtered.add(item);
            }
